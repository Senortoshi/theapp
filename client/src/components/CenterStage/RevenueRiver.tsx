...
export const RevenueRiver = React.memo(function RevenueRiver() {
  ...
  const contributorIdToX = useMemo(() => {
    ...
    const spread = Math.min(width - 40, Math.max(0, contributors.length * (isMobile ? 24 : 36)));
    ...
    return m;
  }, [contributors, width, isMobile]);

  useRevenueRiver(canvasRef, contributorIdToX, width, HEIGHT, isMobile);
